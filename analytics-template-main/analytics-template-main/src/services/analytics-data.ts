...
export interface TrackMapResult {
  track: string;
  year: number;
  session: string;
  title: string;
...
}): Promise<{ data: StandingRow[] }> => {
  await new Promise((resolve) => setTimeout(resolve, 100));

  // Mock standings data — replace with real API integration when available
  const sample: StandingRow[] = [
    { position: 1, driver: "Max Verstappen", team: "Red Bull", points: 395 },
    { position: 2, driver: "Lewis Hamilton", team: "Mercedes", points: 275 },
    { position: 3, driver: "Charles Leclerc", team: "Ferrari", points: 230 },
    { position: 4, driver: "Lando Norris", team: "McLaren", points: 200 },
    { position: 5, driver: "George Russell", team: "Mercedes", points: 185 },
  ];

  return { data: sample };
};

export const getTrackMapData = async (params?: {
  year?: number;
  track?: string;
  session?: string;
}): Promise<TrackMapResult> => {
  const year = params?.year ?? 2023;
  const track = params?.track ?? "Silverstone";
  const session = params?.session ?? "Q";
  ...
  if (!response.ok) {
    throw new Error(`Failed to load track map data (${response.status})`);
  }

  return (await response.json()) as TrackMapResult;
};
